    ...
    function checkAndDecrypt() {
        ...
        try {
            ...
            function getFifthRoot(n) {
                ...
                while (true) {
                    let nextX =
                        (BigInt(4) * x + n / x ** BigInt(4)) / BigInt(5);
                    if (nextX >= x) return x;
                    x = nextX;
                }
            }
            ...
            for (let i = 0; i < parts.length; i++)
                bigNum = bigNum * BigInt(1000) + BigInt(parts[i]);
            ...
            for (let i = 0; i < indices.length; i++) {
                const v = indices[i];
                b64 +=
                    v < ebcdicLayout.length && ebcdicLayout[v] !== " "
                        ? ebcdicLayout[v]
                        : String.fromCharCode(v - 100);
            }
            resDiv.innerText = decodeURIComponent(atob(b64));
        } catch (e) {
            resDiv.innerText =
                "❌ 解読失敗：データまたは鍵が正しくありません。";
        }
    }
